...
$\mathcal{L}$ è il linguaggio macchina di $\mathcal{M_{L}}$, un
programma non è altro che una sequenza di istruzioni in $\mathcal{L}$,
...
condizione. Esistono due tipi di iterazione: \begin{enumerate}\item
...
il ciclo fetch/decode/load/execute/save.\end{small}). \\ $\forall i\epsilon\mathcal{D},\mathcal{I_{L}^{L_{O}}}(\mathcal{P^{L}},i)=\mathcal{P^{L}}(i)$\\
...
\url{https://en.wikipedia.org/wiki/Data_segment}} di un programma, il momento di allocazione può cadere o durante il
\textbf{run-time }o il \textbf{compile-time. }\\ Esistono 3 tipi
