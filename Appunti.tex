...
e memorizzare programmi. Questo insieme permette di definire ed implementare
un linguaggio. \\$\mathcal{M_{L}}$ identifica una macchina astratta
che capisce ed esegue il linguaggio $\mathcal{L}$. $\mathcal{L}$
è il linguaggio macchina di $\mathcal{M_{L}}$, un programma non è
altro che una sequenza di istruzioni in $\mathcal{L}$, $\mathcal{P^{L}}$
è un programma scritto in linguaggio $\mathcal{L}$.
...
\textbf{produce e/o modifica }un valore o uno stato al di fuori del
proprio \textbf{scope locale.}
...
slide 27 in poi}\\ Le espressioni sono le principali cause di \textbf{overflow }e/o
di \textbf{underflow}, grazie alle operazioni matematiche che come
risultato riportano un numero troppo grande o troppo piccolo. \\\textbf{
...
da funzioni matematiche. Esse non possono non avere un valore, grazie
al fatto che non esistono funzioni matematiche con codominio vuoto,
quindi tutte le funzioni \textbf{void} ritornano un unico valore (non
ci è dato sapere quale).
...
(locazioni di memoria) \end{itemize} Le variabili hanno diverse proprietà
in base al tipo di linguaggio in cui si trovano: \begin{itemize}\item
...
dell'assegnamento di uno spazio di memoria. 
\begin{lstlisting}
...
Oggetto l;
Oggetto s;
...
...
\end{lstlisting}
 \begin{small} s punta allo stesso spazio in memoria di l \end{small}
\item \textbf{Linguaggi funzionali:} non presentano variabili modificabili.
\end{itemize}
...
Scope dinamico\\ Lo \textbf{scope dinamico,} per risolvere una variabile
cerca il più recente legame ancora attivo stabilito a tempo di esecuzione,
questo scope è più semplice da implementare ma è meno efficiente e
meno leggibile.
...
Segment di un programma, il momento di allocazione può cadere o durante
il \textbf{run-time }o il \textbf{compile-time. }\\ Esistono 3 tipi
di allocazione della memoria: \begin{itemize}\item Statica \item
Dinamica mediante stack\item Dinamica mediante heap.\end{itemize}
...
di ritorno \item l'indirizzo del risultato \item e i parametri \item
...
il puntatore dello heap allo spazio così liberato. \item(\textbf{Best
...
spostato sul blocco sucessivo. Alla deallocazione il blocco viene
reinserito nella lista. \begin{itemize}\item (\textbf{Buddy system})
Il \textbf{buddy system }suddivide i blocchi in potenze di 2, quando
viene richiesto un blocco di dimensione $\mathit{n}$ si cerca il
più piccolo blocco libero tale che $2^{k}\geqslant\mathit{n}$ \item(\textbf{Heap
di Fibonacci}) In questo caso la suddivisione è uguale al \textbf{buddy
...
o a dimensione fissa) possono essere entrambi soggetti a \textbf{due}
...
nuovi. Ciò potrebbe rappresentare un problema per la cache\end{itemize}\item
...
vengono riallocati in maniera che siano tutti contigui \end{itemize}
...
\\ \textbf{Ereditarietà multipla con replicazione:} 
\begin{lstlisting}
B extends A
C extends A
D extends B,C
...
di dato diversi ma compatibili \end{itemize} \textbf{Shadowing:}
definizione di una variabile di istanza che con lo stesso tipo di
una variabile della classe padre. Questo fa si che la variabile del
padre non sia direttamente accessibile alla classe figlio.
...
\bibitem{key-3} Corso di programmazione in C di Claudio Fornaro:
\href{file:/Riferimenti/18-DynAlloc.pdf}{/Riferimenti/18-DynAlloc.pdf}

\bibitem{key-4} Linguaggi Sentassi e Semantica di Alfonso Miola:
