...
il linguaggio $\mathcal{L_{O}}$\\ Le CPU microprogrammate sono CPU
di cui è possibile aggiornare il firmware. E' presente un microinterprete
di microistruzioni che esegue il ciclo fetch/decode/load/execute/save.\\
Quando due diversi nomi denotano lo stesso oggetto si sta verificando
un effetto chiamato \textbf{aliasing}. \\ Una \textbf{dichiarazione
}è un meccanismo con il quale si crea un'associazione nell'ambiente.
\\ Il \textbf{binding }è il nome dato alla creazione di un'associazione
tra nome ed un oggetto denotabile. \\ Un \textbf{identificatore }è
una sequenza di caratteri usata per denotare qualcosa. \\ Il primo
\textbf{costrutto if }è stato introdotto nel linguaggio \textbf{Algol
...
e memorizzare programmi.\\ Come per le macchine fisiche (CPU) ad
ogni macchina astratta è associato un suo linguaggio \\$\mathcal{M_{L}}$
identifica una macchina astratta che capisce ed esegue il linguaggio
$\mathcal{L}$. $\mathcal{L}$ è il linguaggio macchina di $\mathcal{M_{L}}$,
un programma non è altro che una sequenza di istruzioni in $\mathcal{L}$

\subsubsection*{Macchina astratta ad implementazione compilativa}
\begin{itemize}
...
\subsubsection*{Macchina astratta ad implementazione interpretativa}
\begin{itemize}
...
implementa il ciclo fetch/decode/load/execute/save. \\ $\forall i\epsilon\mathcal{D},\mathcal{I_{L}^{L_{O}}}(\mathcal{P^{L}},i)=\mathcal{P^{L}}(i)$\\
Per qualsiasi input $i$, l'interprete applicato a $\mathcal{P^{L}}$
ed $i$ ritorna lo stesso risultato ritornato da $\mathcal{P^{L}}$
applicato ad $i$\\ $\mathcal{I_{L}^{L_{O}}}$identifica un interprete
$\mathcal{I}$ del linguaggio $\mathcal{L}$ scritto nel linguaggio
$\mathcal{L_{O}}$
...
\\ Un compilatore può anche trasformare una ricorsione in coda in
modo da farla diventare una iterazione. \\ \begin{small}\textbf{
...
piccole, se è una ricorsione incontrollata può incorrere in un \textbf{stack
overflow}. Esiste una variante chiamata \textbf{ricorsione in coda},
la quale consiste nel chiamare la funzione che ritorna il valore senza
fare ulteriori computazioni. \\\textbf{ Iterazione:} si realizza
quando si eseguono più righe di codice al verificarsi di una determinata
condizione. Presenta due tipi di iterazione: \begin{enumerate}\item
\textbf{Iterazione determinata:} il numero di volte che il ciclo è
eseguito è \textbf{stabilito a priori} e non può essere cambiato \item
\textbf{Iterazione indeterminata:} il ciclo viene ripetuto \textbf{fino
a }quando una qualche espressione diventa \textbf{falsa} \end{enumerate} \end{small}

\subsection*{Linguaggio di programmazione}

$\mathcal{L}$ identifica un linguaggio macchina.\\ $\mathcal{P^{L}}$
denota un programma scritto in linguaggio $\mathcal{L}$
...
un tipo equivale a se stesso \item un \textbf{alias} di un tipo è
equivalente a quel tipo \item Se due tipi sono costruiti applicando
lo stesso costruttore di tipo a tipi equivalenti, allora essi sono
equivalenti. \end{itemize}
...
\item Si ha \textbf{poliformismo universale parametrico} quando un valore
ha un'infinità di tipi diversi che si ottengono da un unico schema
di tipo generale.
...
\item Un sistema di tipi viene detto \textbf{polimorfo} quando uno stesso
oggetto può avere più di un tipo
\item Un valore esibisce \textbf{polimorfismo di sottotipo }(o \textbf{limitato})
quando ha un'infinità di sottotipi diversi, che si ottengono da uno
schema di tipo generale, sostituendo ad un opportuno parametro i sottotipi
di un tipo assegnato (\textbf{extends }in Java)
\item Un sistema di tipi viene detto \textbf{monomorfo} quando ogni oggetto
del linguaggio ha un unico tipo
...
di colonna}, in questo caso l'array è memorizzato nei primi/second/terzi...
...
di \textbf{colonna} perchè essa rende semplice lo \textbf{slicing}
dell'array.
...
Lo \textbf{scope statico }per risolvere una variabile esamina partendo
dal blocco interno fino ad arrivare a quello esterno.\\ Lo \textbf{scope
statico} è più efficiente, le associazioni sono note a tempo di esecuzione
ma è molto più complesso da implementare. \\ Presenta \textbf{il
puntatore a catena statica}, esso punta al blocco immediatamente esterno
a quello chiamato, così facilita la ricerca dei riferimenti ai \textbf{binding}
non locali. \\ Viene creato anche il \textbf{display} cioè un vettore
che memorizza un record di attivazione per ciascun blocco di annidamente;
ciò serve a ridurre il costo di scansione della catena statica. Quando
ci si riferisce ad un oggetto non locale, il suo puntatore viene reperito
dal vettore (così non serve cercare in tutta la \textbf{catena statica}).

\subsubsection*{Scope dinamico}

Lo \textbf{scope dinamico} per risolvere una variabile cerca il più
recente legame ancora attivo stabilito a tempo di esecuzione, è più
semplice da implementare ma è meno efficiente e meno leggibile.

\subsection*{Ambiente e sottodivisioni}

Un \textbf{ambiente }è un insieme di associazioni fra nomi ed oggetti
denotabili esistenti ad uno specifico punto dell'esecuzione del programma.
\begin{itemize}
...
\end{itemize}
Una funzione produce un \textbf{effetto collaterale} quando la funzione
\textbf{produce/modifica }un valore o uno stato al di fuori del proprio
\textbf{scope locale.}

\subsubsection*{Blocco}

Il \textbf{blocco} è una regione testuale di un programma avente un
inizio e una fine, esso può contenere dichiarazioni locali alla regione.

\subsection*{Chiamata e ritorno da una procedura}

Sequenza di operazioni che avvengono per realizzare una chiamata ad
una procedura(funzione):

\begin{enumerate}\item Modifica del programma counter \item Allocazione
dello spazio sulla pila \item Modifica del puntatore del record di
attivazione \item Passaggio dei parametri \item Salvataggio dei
registri \item Esecuzione del codice per l'inizializzazione (se presente)
\end{enumerate}

Sequenza di operazioni che avvengono al ritorno da una chiamata ad
una procedura:

\begin{enumerate}\item Ripristino del program counter \item Restituzione
dei valori di ritorno \item Ripristino dei registri \item Esecuzione
codice per la fializzazione (se esiste) \item Deallocazione dello
spazio sulla pila \end{enumerate}
...
\end{enumerate} Presenta anche due tipi di \textbf{valutazione: }\begin{enumerate}\item
...
di una stringa in accordo a determinate forme grammaticali. \footnote{figura mancate}\\\textbf{
Chiusura:} una coppia di ambiente e espressione in cui questa dev'essere
valutata. L'ambiente deve comprendere (almeno) tutte le variabili
libere dell'espressione.\\ Le espressioni solo le principali cause
di \textbf{overflow }e/o di \textbf{underflow}, grazie alla somma/sottrazione/prodotto/divisione
che come risultato riporta un numero troppo grande o troppo piccolo.

\subsubsection*{Passaggio di parametri}

Esistono vari tipi di passaggio di parametro ad una funzione: \begin{itemize}\item\textbf{Passaggio
...
con \textbf{l-value} passata\end{itemize}

Si possono passare anche le funzioni come parametro, durante questa
valutazione i nome non locali possono essere cercati in due modi:
\begin{itemize}\item \textbf{Deep binding:} nel blocco in cui la
funzione passata come parametro è stata dichiarata (Java) \item \textbf{Shallow
binding:} nell'ambiente della funzione passata (linguaggio con scope
dinamico)\item \textbf{Passaggio di parametri per valore:} viene
valutato il \textbf{r-value} del parametro ed il valoe viene passato
alla funzione. Nel caso i parametri siano delle strutture il passaggio
per valore crea una copia campo-campo di quella struttura (costo notevole
per strutture di grandi dimensioni).\item \textbf{Passaggio di parametri
per costante:} avviene lo stesso tipo del \textbf{passaggio per valore}
ma il valore non può essere modificato. Nel caso i parametri siano
delle strutture il passaggio per costante passa un riferimento alla
struttura e non permette che questa possa in qualche modo essere modificata.\item
\textbf{Passaggio di parametri per riferimento:} viene passato alla
funzione il \textbf{l-value} del parametro. \end{itemize}

\subsubsection*{Notazione}

Notazione infissa\\La notazione infissa consiste nel mettere l'operatore
tra i due elementi ($a+b$). Questo tipo di notazione è difficile
da realizzare rispetto alle altre notazione visto che necessita di
conoscere l'ordine di precedenza degli operandi.\\Notazione prefissa\\La
notazione prefissa consiste nel mettere l'operando prima dei due elementi
($+a\quad b$).\\Notazione postfissa\\La notazione postfissa consiste
nel mettere l'operando dopo i due elementi dell'espressione ($a\quad b+$).
...
comando \textbf{void} ritorni un unico valore invece che nessun valore,
perchè essendo le funzioni dei linguaggi di programmazione derivate
da funzioni matematiche. Essa non può non avere valore perchè non
...
(locazioni di memoria) \end{itemize} Esistono anche altri tipi di
variabili che cambiano in base al tipo di linguaggio: \begin{itemize}\item
\textbf{Linguaggi iperativi: }\begin{small} \begin{itemize}\item
variabili modificabili \item aree di memoria associate ad un nome,
che possono contenere un valore memorizzabile \item \textbf{doppio
mapping:} nome $\rightarrow$ variabile e variabile $\rightarrow$
valore memorizzato \end{itemize} \end{small} \item \textbf{Linguaggi
orientati agli oggetti:} \begin{small} \begin{itemize}\item variabili
con \textbf{modello a riferimento} \item riferimenti (l-value) ad
oggetti memorizzati nello heap \item \textbf{assegnamento:} non modifica
l'oggetto riferito, ma cambia il riferimento \end{itemize}\end{small}\item
\textbf{Linguaggi funzionali:} non presentano variabili modificabili.
\end{itemize}
...
La memoria può essere allocata nello stack , nello heap o nel Data
Segment di un programma, il moTipimento di allocazione può accadere
o durante il \textbf{run-time }o il \textbf{compile-time. }

Ci sono 3 tipi di allocazione della memoria: \begin{itemize}\item
Statica \item Dinamica mediante stack\item Dinamica mediante heap.\end{itemize}
...
e non può nemmeno essere rilasiata. Questa variabile viene definita
durante il \textbf{compile-time} ed allocata nel \textbf{Data-segment}. 

Un oggetto che viene \textbf{allocato staticamente} mantiene uno stesso
indirizzo d memoria per tutta l'esecuzione del programma.
...
quinidi è si modificabile e rilasciabile ma solo con chiamate opportune
da parte del programmatore.

\subsection*{Stack \& Heap}

Nella scelta della memoria tra \textbf{stack }e \textbf{heap} dipende
sia dal compilatore sia dal programmatore, per esempi i risultati
...
reverse order, thus restoring the previous values of all the variables.\footnote{Descrizione presa da: \href{https://www.quora.com/What-is-the-difference-between-the-stack-and-the-heap}{What is the difference between the stack and the heap?}}\end{quote}

\textbf{RDA (stack frame): }lo \textbf{stack frame} memorizza nello
stack tutte le informazioni relative all'istanza di un sottoprogramma.\begin{quote} 
...
Durante una funzione nello \textbf{stack frame} vengono memorizzati:\begin{itemize}\item Risultati intermedi\item Variabili locali\item Puntatore
di catena dinamica:\begin{small} punta al record di attivazione precedente;
dal momento che i record di attivazione hanno tutti una dimensione
diversa, bisogna tenere traccia di dove inizia lo \textbf{stack frame
precedente}, in modo da poterlo ripristinare in seguito.\end{small}
\item Puntatore di catena statica (se viene implementato scope statico)
\item Indirizzo di ritorno \item Indirizzo del risultato \item
Parametri \item Nel caso di un metodo anche un riferimento all'oggetto
su cui il metodo viene chiamato \end{itemize}

Invece nel caso di \textbf{stack frame con blocchi anonimi} vengono
memorizzati:

\begin{itemize}\item Puntatore di catena dinamica \item Variabili
locali \item Risultati intermedi delle espressioni \end{itemize}
...
in an arbitrary sequence instead of strictly in reverse order.\footnote{Descrizione presa da: \href{https://www.quora.com/What-is-the-difference-between-the-stack-and-the-heap}{What is the difference between the stack and the heap?}}\end{quote}
\begin{enumerate}
...
libera}) Quando viene richieste della memoria si restituisce un blocco
di dimenzione più appropriata e lo si rimuove dalla lista. Quando
invece la memoria viene liberata, essa viene riaggiunta alla lista
e fusa con i blocchi liberi adiacenti in modo da formare un unicoblocco
di memoria libera. \item (\textbf{Compattazione della memoria libera})
Lo heap ogni volta che viene richiesta memoria viene suddiviso continuando
dalla allocazione precedente senza liberare lo spazio prima (anche
in caso di deallocazione dell'oggetto), ma quando finisce la memoria
di heap libera, questo metodo ``\textbf{compatta}'' i blocchi allocati,
creando una zona continua di spazio. Alla fine aggiorna il puntatore
dello heap allo spazio così liberato. \item(\textbf{Best fit}) La
strategia \textbf{best fit} consiste nello scorrere tutta la lista
alla ricerca del blocco che combacia meglio con la dimensione del
blocco richiesto\item(\textbf{First fit}) La strategia \textbf{first
...
Questo tipo di gestione dello \textbf{heap} è soggetto a \textbf{due}
tipi di \textbf{frammentazione:}\begin{enumerate}\item\textbf{ interna},
...
totale sono presenti, la memoria disponibile è sparsa per tutto lo
heap.\end{enumerate}
\item \textbf{Heap a blocchi di dimensione fissa:} lo \textbf{heap} è costituito
da una lista di $\mathrm{n}$ blocchi di dimensione fissa. Quando
viene richiesta della memoria, il primo elemento libero viene rimosso
dalla lista e restituito al richiedente, mentre il puntatore viene
spostato sul bloccosucessivo. Alla deallocazione il blocco viene reinserito
nella lista. \begin{itemize}\item (\textbf{Buddy system}) Il \textbf{buddy
system }suddivide i blocchi in potenze di 2, quando viene richiesto
un blocco di dimensione $\mathit{n}$ si cerca il più piccolo blocco
libero tale che $2^{k}\geqslant\mathit{n}$ \item(\textbf{Heap di
Fibonacci}) In questo caso la suddivisione è uguale al \textbf{buddy
system} ma la dimensione dei blocchi allocabili segue la sequenza
di Fibonacci e quindi crescono di dimensione più lentamente.\end{itemize}
\end{enumerate}
...
di uguali dimensione. Solo una delle parti è attiva alla volta. Quando
la prima parte sta per esaurirsi, viene fatta una visita di tutti
gli oggetti allocati nella heap e quelli validi vengono copiati nella
...
in uso. Una volta fatto, vengono liberati tutti i blochhi non marcati.
\\ \textbf{Svantaggi:} \begin{itemize}\item Causa frammentazione
esterna \item Richiede tempo proporzionale alla dimensione dello
heap, indipendentemente da quale sia la percentuale di questo sia
effettivamente utilizzata \item Le posizioni degli oggetti deallocati
vengono spesso riciclate, cos' facendo gli oggetti vecchi si trovano
vicino a quelli nuovi. Ciò potrebbe rappresentare un problema per
la cache\end{itemize}\item \textbf{Mark and compact:} funziona allo
stesso modo del \textbf{mark and sweep}, solo che nella fase di \textbf{sweep}
gli oggetti in memoria vengono riallocati in maniera che siano tutti
contigui \end{itemize}
...
La \textbf{allocazione statica} alloca sempre tutta la memoria che
potrebbe essere necessaria, per cui potrebbe essere allocata memoria
anche per le procedure che non verranno richiamate durante l'esecuzione
...
si basavano le classi figli \item Devono essere ricompilate tutte
le classi figlie in quanto devono essere aggiornati gli \textbf{offset}
ai quale trovare le variabili \end{itemize} \textbf{Ereditarietà
multipla:} quando si estendono due classi che hanno un metodo con
la stessa firma, determinare il metodo giusto da chiamare è difficile
\\ \textbf{vTable:} in un linguaggio ad oggetti con tipi statici
una \textbf{vtable} è una struttura dati che tiene traccia di tutti
i metodi definiti, ridefiniti o ereditati di una classe \\ \textbf{Problemi
...
di dato diversi ma compatibili \end{itemize} \textbf{Ereditarietà
multipla con replicazione:} 
\begin{lstlisting}
B extends A
C extends A
D extends B,C
\end{lstlisting}
 \textbf{Shadowing:} definizione di una variabile di istanza che con
lo stesso tipo di una variabile della classe padre. Questo fa si che
la variabile del padre non sia direttamente accessibile alla classe
figlio.
\begin{thebibliography}{1}
\bibitem{key-1}Appunti basati sul lavoro di Slavetto: \url{https://slavetto.github.io/primo-anno/}
codice git: \url{https://github.com/slavetto/slavetto.github.io}

\bibitem{key-2} Integrazione con le slide di Luca Albeni: \href{file:/Riferimenti/SL_Slide_Luca_Albeni.zip}{SL\_Slide\_Luca\_Albeni.zip}
\end{thebibliography}
